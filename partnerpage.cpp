...
PartnerPage::PartnerPage(QWidget *parent)
    : QWidget(parent)
...
//*********************/
/*
QVector<bool> PartnerPage::getMatchResultsForTeam(int teamId)
{
    QVector<bool> results;

    QSqlQuery query;
    query.prepare("SELECT WINNINGTEAMID FROM Matchs WHERE TEAM1ID = :id OR TEAM2ID = :id ORDER BY MATCHID DESC");
    query.bindValue(":id", teamId);

    if (query.exec()) {
        while (query.next()) {
            int winningTeamId = query.value(0).toInt();
            results.append(winningTeamId == teamId); // true si victoire
        }
    } else {
        qDebug() << "Erreur lors de l'exécution de la requête Matchs:" << query.lastError().text();
    }
    ...
    if (query.exec()) {
        while (query.next()) {
            ...
            if (isWin) {
                ...
                break; // Fin de la série consécutive
            }
        }
    }

    return qMakePair(totalWins, currentStreak);
}
