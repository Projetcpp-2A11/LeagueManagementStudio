...
teams::teams(int id, QString name, QString logo, QString contry)
    :teamID(id),name(name),contry(contry),logo(logo)
{}
...
teams teams::getTeamById(int id)
{


    QSqlQuery query;
    query.prepare("SELECT NAME,LOGO,COUNTRY FROM TEAMS WHERE TEAMID= :id ");
    query.bindValue(":id", id);
    if (query.exec() && query.next()) {
        QString name = query.value("NAME").toString();
        QString logoPath = query.value("LOGO").toString();
        QString country = query.value("COUNTRY").toString();

        return teams(id, name, logoPath,country);
    }
    else {
        qDebug() << "Failed to find team:" << query.lastError().text();
        return teams();
