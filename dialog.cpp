...
QString Dialog::getSortOrder() const
{
    QString text = ui->filtre->currentText().trimmed().toLower();
    qDebug() << "Texte du tri détecté :" << text;

    // Vérifier "décroissante" en premier
    if (text.contains("décroissante")) {
        return "DESC";
    } else if (text.contains("croissante")) {
        return "ASC";
    } else {
        return "DESC"; // Valeur par défaut
    }
}
...
void Dialog::applyFilters() {
    ...
    QString sortOrder = getSortOrder();

    // Créez la liste AVANT de l'utiliser
    QList<QString> filters;
    filters.append(startDate.toString("yyyy-MM-dd"));
    filters.append(endDate.toString("yyyy-MM-dd"));
    filters.append(stadium);
    filters.append(matchResult);
    filters.append(getSortOrder()); // Index 4
